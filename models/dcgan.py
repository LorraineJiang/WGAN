...
class DCGAN_D(nn.Module):
    ...
    def forward(self, input):
        ...
        output_mean = output.mean(0)     # 按列求平均值,返回的形状是(1，列数)
        return output, output_mean.view(1)       # view函数把output转换为1×1的数
...
class DCGAN_G(nn.Module):
    ...
    def forward(self, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            output = self.main(input)
        # 生成器没有求mean这一步,因为目标函数中会把生成样本传进判别器判断后再求mean
        return output 
###############################################################################
class DCGAN_D_nobn(nn.Module):
    ...
    def forward(self, input):
        ...
        return output, output_mean.view(1)
